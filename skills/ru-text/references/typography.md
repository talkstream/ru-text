...
R30. После в, к, с, о, у, и, а, я -- неразрывный пробел (U+00A0), чтобы они не оказывались на конце строки. `в<nbsp>Москве`, `к<nbsp>сожалению`, `я<nbsp>думаю`

⚠ Заголовок говорил «предлогов/союзов», а список включает «я» -- местоимение. Верен список: Ководство § 62 запрещает оставлять на строке «одно-, двух- и некоторую часть трехбуквенных слов», а не только служебные. Обе редакции родились в одном коммите и дожили до v1.4.0; три сокращённые выжимки, где «я» потерялось, повторяли ту же узкую формулировку -- что они списаны с заголовка, не показано.
...
⚠ R49 и R50 смотрят на ОДНО И ТО ЖЕ место с разных сторон, и порядок между ними задаёт регистр справа. Строчная буква или цифра — предложение продолжается, пробелы вокруг многоточия убираются (R49). Заглавная — начинается новое предложение, и пробел, наоборот, нужен (R50). Без этого разделения правила съедали бы правки друг друга: одно ставит пробел, другое его убирает, и текст не сходится ни к какому виду.

⚠ Три случая, где пробел вокруг многоточия остаётся, хотя правило будто требует обратного:

- **Диапазон.** `А…Я`, `10…20 °C` — слева не слово, а одиночный индекс или цифра. Вставленный пробел разорвал бы диапазон надвое.
- **Реплика диалога.** `— Я думал … — Не думал, — отрезал он.` Справа от многоточия тире, а не буква. Съесть пробел перед репликным тире значит произвести ошибку там, где была неровность.
- **Пустое поле бланка.** `Фамилия … Имя … Отчество …` Многоточие здесь — место под заполнение, а не обрыв фразы; склейка превратила бы бланк в оборванный текст.

⚠ R47 и R48 остаются НЕ реализованными машинно, и причина в них самих: чтобы снять пробел перед многоточием в конце предложения, надо отличить конец предложения от пропуска в цитате, где пробел уместен. Механического признака этого различия корпус не даёт — правило держит читатель.
...
⚠ Одну запятую снимать нельзя, и это не осторожность, а соседнее правило: R54 требует запятую как ДЕСЯТИЧНЫЙ разделитель. Поэтому `1,500` законен сразу по двум чтениям — «полторы тысячи» по R53 и «одна целая пятьсот тысячных» по R54, — и различить их нечем ни контекстом, ни числом цифр. Неоднозначность снимают две и более групп: десятичная запятая в числе ровно одна. `1,500,000` -> `1 500 000`, а `1,500` остаётся как есть.
...
⚠ Точка снимается не всегда, и оговорка тут несущая: та же точка закрывает предложение. Снимать её можно только когда предложение ПРОДОЛЖАЕТСЯ — то есть справа стоит строчная буква: `Вес 5 кг. брутто` -> `Вес 5 кг брутто`, но `Вес 5 кг. Брутто указано` остаётся как есть. Конец текста тоже не продолжение: `Вес 5 кг.` не трогаем.

⚠ И только многобуквенные единицы — `кг`, `км`, `мин`, `га`. Однобуквенные `с`, `м`, `л`, `ч` из списка выше с точкой означают другое: `с.` страница, `л.` лист, `ч.` часть. Буквы `г` в списке R74 нет вовсе, и это удача: `г.` — год и город, а `в 2020 г` уже другой текст. Удаление необратимо для смысла, поэтому список сужен, а не расширен.
