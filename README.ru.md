...
**Плагин для Claude Code, Codex CLI, Gemini CLI, Cursor, OpenClaw и Notion — качество русского текста** — типографика, информационный стиль, редактура, UX-тексты и деловая переписка.
...
Работает с Claude Code, Codex CLI, Gemini CLI, Cursor, OpenClaw и Notion.
...
Также доступен на [ClawHub](https://clawhub.ai/skills/ru-text). Работает с любым LLM-провайдером и каналом, который поддерживает OpenClaw.
